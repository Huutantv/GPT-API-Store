...
function latestUserAsksModelIdentity(messages) {
  if (!Array.isArray(messages)) return payloadHasModelIdentityQuestion(messages);
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    const message = messages[i];
...
app.post("/api/credit/adjust-token", (req, res) => {
  const admin = checkAdminAuth(req);
  if (!admin.ok) return res.status(admin.status).json({ detail: admin.message });
  const body = req.body || {};
  const key = String(body.key || "").trim();
