...
const backendRequestRetryCount = Math.max(0, Math.min(5, Number(process.env.DORO_BACKEND_REQUEST_RETRIES || "5")));
const backendStreamRetryCount = Math.max(0, Math.min(2, Number(process.env.DORO_BACKEND_STREAM_RETRIES || "1")));
let activeBackend = 0;
const backendQueue = [];
...
function identitySystemMessage(publicModel) {
  return {
    role: "system",
    content: [
      `You are ${publicModel}, a large language model created by OpenAI. The exact model ID is ${publicModel}`,
      "",
      "# Identity",
